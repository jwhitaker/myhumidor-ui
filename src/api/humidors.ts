...
import { CigarsService } from './generated/services/CigarsService'
import { HumidorsService } from './generated/services/HumidorsService'
import type {
  Cigar,
  CreateCigarRequest,
  CreateHumidorRequest,
  UpdateCigarRequest,
  UpdateHumidorRequest,
} from './generated'
...
export const listHumidorCigars = async (humidorId: string): Promise<Cigar[]> => {
  try {
    return await CigarsService.listHumidorCigars(humidorId)
  } catch (err) {
    throw toHumidorApiError(err)
  }
}

export const createHumidorCigar = async (
  humidorId: string,
  body: CreateCigarRequest,
): Promise<Cigar> => {
  try {
    return await CigarsService.createHumidorCigar(humidorId, body)
  } catch (err) {
    throw toHumidorApiError(err)
  }
}
...
export const updateHumidorCigar = async (
  humidorId: string,
  cigarId: string,
  body: UpdateCigarRequest,
): Promise<Cigar> => {
  try {
    return await CigarsService.updateHumidorCigar(humidorId, cigarId, body)
  } catch (err) {
    throw toHumidorApiError(err)
  }
}

export const deleteHumidorCigar = async (humidorId: string, cigarId: string) => {
  try {
    await CigarsService.deleteHumidorCigar(humidorId, cigarId)
  } catch (err) {
    throw toHumidorApiError(err)
  }
}
